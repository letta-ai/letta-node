...
export class Agents {
     ...
     *         }])
     */
    public async createBatchMessageRequest(
        request: Letta.LettaBatchRequest[],
        requestOptions?: Agents.RequestOptions,
    ): Promise<Letta.LettaBatchResponse> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
     ...
     */
    public async retrieveBatchMessageRequest(
        batchId: string,
        requestOptions?: Agents.RequestOptions,
    ): Promise<Letta.LettaBatchResponse> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.LettaEnvironment.LettaCloud,
                `v1/agents/messages/batches/${encodeURIComponent(batchId)}`,
            ),
            method: "GET",
            headers: {
                "X-Fern-Language": "JavaScript",
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.LettaTimeoutError(
                    "Timeout exceeded when calling GET /v1/agents/messages/batches/{batch_id}.",
                );
            case "unknown":
                throw new errors.LettaError({
                    message: _response.error.errorMessage,
