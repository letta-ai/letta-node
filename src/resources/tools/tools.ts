...
import * as ComposioAPI from './composio/composio';
...
export class Tools extends APIResource {
  composio: ComposioAPI.Composio = new ComposioAPI.Composio(this._client);
   ...
   * Upsert base tools
   */
  upsertBase(options?: RequestOptions): APIPromise<ToolUpsertBaseResponse> {
 ...
 * message tool_return (str): The return value of the tool status
 * (Literal["success", "error"]): The status of the tool call tool_call_id (str): A
 * unique identifier for the tool call that generated this message stdout
 * (Optional[List(str)]): Captured stdout (e.g. prints, logs) from the tool
 * invocation stderr (Optional[List(str)]): Captured stderr from the tool
 * invocation
 */
...
export type ToolUpsertBaseResponse = Array<Tool>;
