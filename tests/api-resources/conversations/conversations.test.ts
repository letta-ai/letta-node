...
describe('resource conversations', () => {
  ...
  test.skip('cancel: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.conversations.cancel('default', { agent_id: 'agent_id' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Letta.NotFoundError);
  });
  ...
  test.skip('fork: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.conversations.fork('default', { agent_id: 'agent_id', hidden: true }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Letta.NotFoundError);
  });
  ...
  test.skip('recompile: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.conversations.recompile('default', {
    dry_run: true,
    agent_id: 'agent_id',
    compaction_settings: {
    clip_chars: 0,
