...
describe('resource accessTokens', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.accessTokens.create({ hostname: 'https://example.com', policy: [{
    id: 'id',
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.accessTokens.list({
    agentId: 'agentId',
    limit: 0,
    offset: 0,
  ...
  test.skip('delete: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.accessTokens.delete('token', { body: {} }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Letta.NotFoundError);
  });
});
