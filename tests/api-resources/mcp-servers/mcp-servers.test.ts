...
describe('resource mcpServers', () => {
  ...
  test.skip('update: only required params', async () => {
    const responsePromise = client.mcpServers.update('mcp_server_id', { config: {
    args: ['string'],
    command: 'command',
    mcp_server_type: 'stdio',
  } });
  ...
  test.skip('refresh: request options and params are passed correctly', async () => {
    ...
    await expect(client.mcpServers.refresh('mcp_server_id', { agent_id: 'agent_id' }, { path: '/_stainless_unknown_path' }))
      .rejects
