...
describe('resource chat', () => {
  ...
  test.skip('createCompletion: only required params', async () => {
    const responsePromise = client.voiceBeta.chat.createCompletion('agent_id', { body: { foo: 'bar' } });
  ...
  test.skip('createCompletion: required and optional params', async () => {
    const response = await client.voiceBeta.chat.createCompletion('agent_id', { body: { foo: 'bar' } });
  });
});
