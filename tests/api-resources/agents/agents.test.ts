...
describe('resource agents', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.agents.retrieve('agent-123e4567-e89b-42d3-8456-426614174000', { include: ['agent.blocks', 'agent.identities'], include_relationships: ['string', 'string'] }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('exportFile: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.agents.exportFile('agent_id', {
  ...
  test.skip('importFile: only required params', async () => {
    const responsePromise = client.agents.importFile({ file: await toFile(Buffer.from('Example data'), 'README.md') });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('recompile: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.agents.recompile('agent-123e4567-e89b-42d3-8456-426614174000', { dry_run: true, update_timestamp: true }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Letta.NotFoundError);
  });
});
