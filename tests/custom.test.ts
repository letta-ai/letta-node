...
const client = new LettaClient({
    environment: LettaEnvironment.LettaHosted,
...
describe("Send message", () => {
    it("Should send a message", async () => {
        ...
        expect(Object.assign(new LettaResponseLettaUsageStatistics(), response.usage).stepCount).toEqual(1);
        expect(response.messages.map((message: { messageType: any }) => message.messageType)).toEqual([
            "reasoningMessage",
            "toolCallMessage",
            "toolReturnMessage",
        ]);
        */
        ...
        const lastUserMessage = [...messages].reverse().find((message) => (message as MessagesListResponseItem).message_type === "user_message") as UserMessageOutput;
    ...
    it("Should send a streaming message", async () => {
        ...
        const messageText = "Hello, how are you today?";
        const response = await client.agents.messages.stream(
            agent.id,
            {
                messages: [
                    {
